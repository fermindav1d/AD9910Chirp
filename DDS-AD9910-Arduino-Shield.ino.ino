...
//Find correct DDS settings, then hard code a version into loop/setup  
void loop ()
{
  ...
  static int LastUpButtonState=1; //initialize button values
...
/************
 * MakeOut - decides which generation function to choose (depending on the value of ModIndex
 ********/
void MakeOut()
{
  /*switch (ModIndex)
  {
  case NONE_MOD_TYPE: //modulation off
  ...
  while(1){
      ...
      //Sweep(100000,1000000,10,0);
      uint32_t StartSweepFreq=5000000;
      uint32_t StopSweepFreq = 5030000;
      Sweep(StartSweepFreq,StopSweepFreq,500,1);
         //    delay(1000);
  }
 //     break;
  //}
...
void DisplayHello()
{
  ...
  display.println("we chirp");
  display.println(" #allin");
